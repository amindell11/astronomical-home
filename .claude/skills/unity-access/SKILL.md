...
Every Unity boot — batch or editor — costs ~2.5–4 GB working set, and the machine sustains about two editors. Boot only when free physical RAM is ≥ ~10 GB (`(Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory / 1MB` → GB). Below that, propose the Alastor remote-gate fallback to the user (it needs their go-ahead, below); otherwise report the memory pressure and wait for an editor to exit.

Run commands from the repository root with PowerShell.

## Alastor remote-gate fallback

**Ask the user before touching Alastor** — every session, even for read-only SSH checks; a past approval does not carry over. The lane has no cross-session checkout guard (#588), so an unannounced run can trample another session's.

When Mordechai is below the local RAM floor and a full batch gate is needed, and the user has said yes: inspect its available RAM, `unity_access.ps1 -Action Status -Json`, and remote `git status`. If its lane is clear, run `scripts/remote_gate.sh <branch>` from the local branch being tested; it owns the bundle/LFS transfer, remote checkout, detached launch, and summary retrieval.
