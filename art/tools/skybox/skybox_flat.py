"""Render starless periodic clouds in Blender and atomically publish a .flatbg bundle.
...
--python-exit-code makes failures nonzero. FLATBG_PATH= names the output.

Bundle v1: manifest.json, image.exr, image.rgba16f. Raw pixels are little-endian
RGBA half floats, scene-linear, bottom row first, width*height*8 bytes. Manifest
contains schema_version, stage, dimensions, pixel_sha256, four RGB palette roles
(base/primary/secondary/accent), and provenance. A single replace publishes all.
"""
...
ASSET_FOLDER = "Assets/Visuals/Environment/Flat/Generated"
MIGRATION = [
...
def save_outputs(scene, preset, out_base, stage, elapsed):
    ...
    image = bpy.data.images.load(str(out_base) + ".exr", check_existing=False)
    try:
        pixels = np.empty(len(image.pixels), dtype=np.float32)
        image.pixels.foreach_get(pixels)
        if not np.isfinite(pixels).all() or np.max(pixels) > 65504:
            raise ValueError("Render exceeds finite RGBAHalf range; reduce emission")
        raw = pixels.astype("<f2").tobytes()
    finally:
        bpy.data.images.remove(image)
    sources = (Path(__file__), Path(skybox_merged.__file__), Path(skybox_preset.__file__))
    manifest = {
        ...
        "provenance": {
            ...
            "generator": "4D torus clouds v1", "samples": scene.cycles.samples,
            "render_seconds": elapsed, "preset": preset, "migration": MIGRATION,
        },
    }
    target = Path(str(out_base) + ".flatbg")
    with tempfile.TemporaryDirectory(dir=target.parent) as staging:
        ...
        with zipfile.ZipFile(staged, "w", compression=zipfile.ZIP_DEFLATED) as bundle:
            ...
            bundle.writestr("image.rgba16f", raw)
            bundle.write(str(out_base) + ".exr", "image.exr")
        os.replace(staged, target)
    scene.render.image_settings.file_format = "PNG"
    scene.render.image_settings.color_mode = "RGBA"
    scene.render.image_settings.color_depth = "8"
    scene.view_settings.view_transform = "AgX"
    bpy.data.images["Render Result"].save_render(str(out_base) + "_preview.png", scene=scene)
    print("FLATBG_MIGRATION=" + json.dumps(MIGRATION))
    print("FLATBG_PATH=" + str(target))
    return target


def publish(bundle_path, project, name):
    project = skybox_unity.project_folder(project)
    with zipfile.ZipFile(bundle_path) as bundle:
        manifest = json.loads(bundle.read("manifest.json"))
        raw = bundle.read("image.rgba16f")
        if manifest["schema_version"] != 1 or manifest["stage"] not in ("draft", "final"):
            raise ValueError("Unsupported flat background manifest")
        if len(raw) != manifest["width"] * manifest["height"] * 8 or hashlib.sha256(raw).hexdigest() != manifest["pixel_sha256"]:
            raise ValueError("Flat background pixels do not match manifest")
    destination = project / ASSET_FOLDER
    destination.mkdir(parents=True, exist_ok=True)
    target = destination / (skybox_unity.sky_name(name) + "-" + manifest["stage"] + ".flatbg")
...
def main():
    ...
    try:
        ...
        bundle = save_outputs(scene, preset, args.out, args.stage, time.perf_counter() - started)
        if args.unity_project:
            print("FLATBG_PUBLISHED=" + str(publish(bundle, args.unity_project, args.name)))
    finally:
        bpy.context.window.scene = previous
        skybox_merged.dispose_scene(scene)
