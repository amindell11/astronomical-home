    ...
    def modal(self, context, event):
        ...
        try:
            ...
            if self.state == "cancelled":
                ...
                return {"CANCELLED"}
            if self.flat:
                bundle = skybox_flat.save_outputs(self.scene, self.preset, self.out_base,
                ...
                published = skybox_flat.publish(bundle, self.unity_project, self.sky_name) if self.send_to_unity else bundle
                settings.status = f"Saved {published.name}. Star settings ignored; see bundle migration report."
...
class SKYBOX_PT_authoring(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if settings.flat_background:
            ...
            layout.label(text="Stretch/rotation remapped; report in bundle.")
        if context.mode != "OBJECT":
