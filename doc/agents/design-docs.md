...
This game is designed in an Obsidian vault checked in at `doc/design/`
(every clone has it; open that folder in Obsidian to edit). Treat these two
as primary context — for design/doc work, and for knowing what to build next:

- **Design vault:** `doc/design/README.md` indexes it; `doc/design/OVERVIEW.md`
  is the game's vision and core pillars (skill-ceiling combat, loot/rewards,
  ...
  follow the doc and call out the conflict. Vault edits land like any other
  change to `doc/` (PR, or the docs-only landing).
...
were measured and rejected, experimental results, rulings, open forks. Vision
— what the game is meant to be, ahead of any code — is the one kind of prose
that is underivable by nature; it lives in the vault (`doc/design/`), and a PR
that changes design-facing behavior updates the note it diverges from. The
why of *decisions* has three homes, none of them under `doc/`:
...
under `doc/` — `doc/agents/`, `Glossary.md`, `Diagnosis_Loop_Cookbook.md`, and
the vault at `doc/design/` — is agent law, vocabulary, and vision, held to the
same bar: derivable text gets cut in the PR that touches it.
