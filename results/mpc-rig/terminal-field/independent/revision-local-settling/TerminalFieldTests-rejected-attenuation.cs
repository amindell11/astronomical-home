...
namespace Tests.EditMode
{
    ...
    public class TerminalFieldTests
    {
        ...
        [Test]
        public void TerminalCost_LeavesLocalRingSettlingToRollout()
        {
            using var grid = new Grid(8, 2f);
            grid.Obstacles[0] = new ObstacleData { position = new float2(4f, 0f), radius = 1f };
            grid.Bake(float2.zero, 1);
            var input = new CostInput
            {
                terminalField = grid.View,
                sentence = new IntentSentence { pos = new PosSlot { armed = true, setpoint = 6f, weight = 1f } },
            };
            var cfg = new Config { wTerminalField = 1f, maxSpeedSq = 16f, horizon = 1, dt = 1f };
            var ring = new State { pos = new float2(6f, 0f) };
            Assert.That(grid.View.Sample(ring.pos), Is.GreaterThan(0f));
            Assert.That(Cost.EvaluateTerminal(ring, input, cfg), Is.Zero);
            var outside = new State { pos = new float2(10f, 0f) };
            Assert.That(Cost.EvaluateTerminal(outside, input, cfg), Is.EqualTo(grid.View.Sample(outside.pos)));
        }
        [Test]
        ...
        private sealed class Grid : IDisposable
        {
            ...
            internal void Bake(float2 goal, int count = 0, float clearance = 0f)
            {
                new TerminalFieldBakeJob
                {
                    ...
                    goal = goal, clearance = clearance, distances = Distances, occupied = Occupied, heap = heap,
                    heapPosition = positions, result = result,
                }.Schedule().Complete();
                View = new TerminalFieldView
                {
                    distances = Distances, valid = true, resolution = size, spacing = spacing, goal = goal,
            ...
            public void Dispose()
            {
                Distances.Dispose(); Occupied.Dispose(); Obstacles.Dispose();
