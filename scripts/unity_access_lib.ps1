<#
.SYNOPSIS
    The Unity access coordinator's implementation, as a function library behind the
    scripts/unity_access.ps1 entry point.

.DESCRIPTION
    Dot-source: . (Join-Path $PSScriptRoot "unity_access_lib.ps1")
    Loading defines functions and constants only: no I/O, no state dirs, no exit.

    Invoke-UnityAccessAction takes the entry point's param block verbatim (a test holds the two
    identical) and returns the action's result object; Get-UnityAccessExitCode maps a result to its
    exit code. The published interface - actions, statuses, exit codes, machine channel, state
    schemas - is the entry point's help; this file adds none.

    Only the entry point and its test load this. An in-process caller becomes the lease holder
    (holderProcessId is its own PID), so every other caller goes through
    scripts/unity_access_client.ps1 (doc/agents/script-contracts.md).
#>
...
#   Dispatch                  Require-Lease, Get-UnityAccessExitCode, Invoke-UnityAccessAction
# ------------------------------------------------------------------------------
...
function Attach-Process {
    param([int]$TargetProcessId = $ProcessId, [string]$RecordEditorProfile = "")
    $owner = Find-OwnerByLease $Lease
    ...
    $owner.processId = $TargetProcessId
    $owner.updatedAt = [datetime]::UtcNow.ToString("o")
    Write-JsonFile (Get-OwnerRecordPath ([string]$owner.projectKey)) $owner
    $boot = Get-BootOwner
    if ($null -ne $boot -and [string]$boot.lease -eq $Lease) {
        $boot.processId = $TargetProcessId
...
function Attach-BatchChild {
    ...
    return Attach-Process -TargetProcessId ([int]$match[0].processId)
}
...
function Start-TrackedEditor {
    ...
    try {
        ...
        $attached = Attach-Process -TargetProcessId $process.Id -RecordEditorProfile $EditorProfile
...
function Get-UnityAccessExitCode {
    param([object]$Result)
    $resultStatus = if ($Result -is [System.Collections.IDictionary]) { [string]$Result["status"] } else { [string]$Result.status }
    return $(if ($statusExitCodes.ContainsKey($resultStatus)) { $statusExitCodes[$resultStatus] } else { 0 })
}

function Invoke-UnityAccessAction {
    param(
