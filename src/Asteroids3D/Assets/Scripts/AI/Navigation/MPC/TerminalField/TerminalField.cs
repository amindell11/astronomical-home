...
namespace AI.Navigation.MPC.TerminalField
{
    ...
    public sealed class TerminalField : IDisposable
    {
        ...
        private NativeArray<float> emptyDistances;
        ...
        public TerminalField(MpcSettings settings, Dynamics dynamics, ObstacleScanner scanner)
        {
            ...
            emptyDistances = new NativeArray<float>(count, Allocator.Persistent);
            ...
            view = new TerminalFieldView { distances = distances, emptyDistances = emptyDistances, occupied = occupied, resolution = settings.terminalFieldResolution };
        }

        public void Update(float dt, float2 ship, bool hasGoal, float2 goal, in Config config, float goalRadius = 0f)
        {
            elapsed += dt;
            ...
            var padding = 2f * view.spacing;
            var lower = view.origin + padding;
            var upper = view.origin + view.spacing * (view.resolution - 1) - padding;
            var keepPlacement = view.valid && math.all(ship - reach >= lower) && math.all(ship + reach <= upper)
                && math.all(goal >= lower) && math.all(goal <= upper);
            if (keepPlacement) spacing = view.spacing;
            var origin = keepPlacement ? view.origin : (ship + goal) * 0.5f - spacing * (view.resolution - 1) * 0.5f;
            var center = origin + spacing * (view.resolution - 1) * 0.5f;
            ...
            new TerminalFieldBakeJob
            {
                ...
                spacing = spacing, origin = origin, goal = goal, goalRadius = goalRadius, clearance = clearance,
                distances = distances, emptyDistances = emptyDistances, occupied = occupied, heap = heap, heapPosition = heapPosition, result = result,
            }.Schedule().Complete();
            view = new TerminalFieldView
            {
                distances = distances, emptyDistances = emptyDistances, occupied = occupied, valid = true, resolution = view.resolution, spacing = spacing,
                origin = origin, goal = goal, goalRadius = goalRadius, seedIndex = result[0].seedIndex,
