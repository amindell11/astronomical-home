...
namespace AI.Navigation.MPC.TerminalField
{
    ...
    public struct TerminalFieldBakeJob : IJob
    {
        ...
        public NativeArray<float> emptyDistances;
        ...
        public void Execute()
        {
            ...
            for (var i = 0; i < distances.Length; i++)
            {
                ...
                for (var j = 0; j < obstacleCount; j++)
                {
                    ...
                    if (math.distancesq(center, obstacle.position) > radius * radius) continue;
                    blocked = true;
                    break;
                }
            ...
            if (goalRadius > 0f)
            {
                var terrainDistances = distances;
                distances = emptyDistances;
                BakeDistances(seed, nearest, false);
                distances = terrainDistances;
            }
            var maximum = BakeDistances(seed, nearest, true);
            result[0] = new BakeResult { seedIndex = seed, maxFiniteDistance = maximum };
        }

        private float BakeDistances(int seed, float nearest, bool blockTerrain)
        {
            heapCount = 0;
            var regionSeeded = false;
            for (var i = 0; i < distances.Length; i++)
            {
            ...
            if (goalRadius > 0f)
                for (var i = 0; i < distances.Length; i++)
                    if (occupied[i] == 0 && math.distancesq(Center(i), goal) <= goalRadius * goalRadius)
                    {
                        distances[i] = 0f;
                        InsertOrDecrease(i);
                        regionSeeded = true;
                    }
            if (!regionSeeded && seed >= 0)
            {
                distances[seed] = math.max(0f, math.sqrt(nearest) - goalRadius);
            ...
            while (heapCount > 0)
            {
                ...
                for (var dx = -1; dx <= 1; dx++)
                {
                    ...
                    if (blockTerrain && occupied[next] != 0 || heapPosition[next] == -2) continue;
                    if (blockTerrain && dx != 0 && dy != 0
