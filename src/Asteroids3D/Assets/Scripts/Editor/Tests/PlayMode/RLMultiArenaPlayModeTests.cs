...
{
    /// <summary>M=2 fan-out leak checks for the in-process multi-arena harness: per-arena provider isolation (each arena's ships sense their own obstacle field), seed decorrelation (arena-1 episode-0 poses differ from arena-0's), spatial confinement (ships never cross into the neighbor's half of the line), and liveness (both async loops complete episodes). Never mirror-determinism — twin-arena identical evolution is a postmortem-rejected gate.</summary>
    [TestFixture]
    [Category("AI")]
    public class RLMultiArenaPlayModeTests
    {
        ...
        public IEnumerator TwoArenas_IsolatedDecorrelatedConfined_BothComplete()
        {
            ...
            var offsetA = TrainingHost.ArenaOffset(0);
            var offsetB = TrainingHost.ArenaOffset(1);
            (hostA, fieldA, pairA, agentA, driverA, projectilesA) = ComposeArena("Arena0", offsetA, in specA);
            (hostB, fieldB, pairB, agentB, driverB, projectilesB) = ComposeArena("Arena1", offsetB, in specB);

            // (a) Provider isolation: each arena's ships were wired to their own field instance.
            Assert.AreNotSame(fieldA.Field, fieldB.Field, "the two arenas must not share a field instance");
            foreach (var ship in new[] { pairA.Agent, pairA.Baseline })
                Assert.AreSame(fieldA.Field, SensedField(ship), $"{ship.name} in arena 0 must sense arena 0's field");
            foreach (var ship in new[] { pairB.Agent, pairB.Baseline })
                Assert.AreSame(fieldB.Field, SensedField(ship), $"{ship.name} in arena 1 must sense arena 1's field");

            // (b) Decorrelation: the arena-1 seed derives different episode-0 poses than arena 0's.
            var relA = GamePlane.WorldPointToPlane(pairA.Agent.transform.position) - offsetA;
            var relB = GamePlane.WorldPointToPlane(pairB.Agent.transform.position) - offsetB;
            ...
            while (aAlive || bAlive)
            {
                ...
                AssertConfined(pairA, offsetA);
                AssertConfined(pairB, offsetB);
        ...
        private (GameObject, HarnessField, EpisodePair, ShipAgent, EpisodeLoopDriver, ProjectileService) ComposeArena(
            string name, Vector2 offset, in RewardSpec spec)
        {
            ...
            var agent = ShipAgentFactory.ComposeHeuristicOnly(pair, brain, in spec, offset, host.transform);
            var driver = new EpisodeLoopDriver(pair, agent, offset, field);
            return (host, field, pair, agent, driver, projectiles);
        }
        ...
        // The commander's sensed field is private state with no accessor; reflection is the only way to pin what a ship was wired to.
        private static IObstacleField SensedField(Ship ship)
        {
            var commander = ship.GetComponentInChildren<AICommander>();
            var sensed = typeof(AICommander).GetField("field", BindingFlags.NonPublic | BindingFlags.Instance);
            return (IObstacleField)sensed.GetValue(commander);
        }

        private static void AssertConfined(EpisodePair pair, Vector2 offset)
        {
            foreach (var ship in new[] { pair.Agent, pair.Baseline })
            {
                if (!ship) continue;
                var planeX = GamePlane.WorldPointToPlane(ship.transform.position).x;
                Assert.Less(Mathf.Abs(planeX - offset.x), TrainingHost.ArenaSpacing * 0.5f,
