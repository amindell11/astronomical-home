...
{
    /// <summary>Committed sample scenario (runner + render smoke, and the living doc for authoring new ones): two policy-pilot ships skirmish for a few seconds. Diagnostics are native gizmos, which only the Game View capture lane films, so the frames this offscreen lane writes carry no markup.</summary>
