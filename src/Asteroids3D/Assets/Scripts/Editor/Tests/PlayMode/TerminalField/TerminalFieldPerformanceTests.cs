...
namespace Tests.PlayMode.TerminalField
{
    ...
    public class TerminalFieldPerformanceTests : PlayModeWorldFixture
    {
        ...
        public IEnumerator WarmedBurstBakeMatrix()
        {
            ...
            report.WriteLine("density,resolution,goalRadius,obstacles,allocationMs,gatherMedianMs,gatherP95Ms,runMedianMs,runP95Ms,scheduleMedianMs,scheduleP95Ms,managedBytes");
                ...
                foreach (var density in new[] { 2f, 2.5f })
                {
                    ...
                    foreach (var goalRadius in new[] { 0f, 18f })
                    {
                        ...
                        var job = new TerminalFieldBakeJob
                        {
                            ...
                            origin = new float2(-half), goal = new float2(0f, 90f), goalRadius = goalRadius, clearance = clearance,
                        ...
                        report.WriteLine(FormattableString.Invariant($"{density},{resolution},{goalRadius},{count},{allocationMs},{gather[100]},{gather[189]},{run[100]},{run[189]},{scheduled[100]},{scheduled[189]},{allocated}"));
                        report.Flush();
        ...
        private static void MeasureSolverLoop(MpcSettings settings, Dynamics dynamics, HarnessField field, string outDir, float density, float goalRadius)
        {
            ...
            try
            {
                ...
                var inputs = new MpcInputs
                {
                    ...
                    sentence = new IntentSentence { pos = new PosSlot { armed = true, referent = 1, weight = 1f, setpoint = goalRadius } },
                ...
                var allocated = GC.GetAllocatedBytesForCurrentThread() - allocatedBefore;
                using var report = new StreamWriter(Path.Combine(outDir, $"solver-loop-d{density}-r{goalRadius}-{DateTime.UtcNow:yyyyMMdd-HHmmss-fff}.csv"));
