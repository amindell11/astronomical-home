...
namespace EnemyAI.RL.Arena
{
    ...
    public class ArenaInstance : BaseGameContext
    {
        ...
        private void HandleShipDamaged(Ship victim, GameObject attacker, float damage)
        {   
            ...
            var shipAttacker = attacker.GetComponentInParent<Ship>();
            if (shipAttacker is null) return;
            // Track damage metrics for TensorBoard
            if (enableMetricsTracking)
            {
                TrackDamageMetrics(victim, shipAttacker, damage);
            }
            ...
            if (!victim.IsFriendly(shipAttacker))
            {
                ApplyDamageReward(shipAttacker, damage, damageInfo, "Reward");
            }
        }
