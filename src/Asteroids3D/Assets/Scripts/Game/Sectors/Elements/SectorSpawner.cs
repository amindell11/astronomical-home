...
{
    /// <summary>
    /// A polymorphic, hand-placed child GameObject that procedurally instantiates content at
    /// sector setup (e.g. a ring of AI ships). Unlike adopt content — where the placed object is
    /// the runtime object — a spawner holds a template + parameters and creates N instances.
    /// Creating a new kind of spawner = subclass this. The spawner's transform is its origin and
    /// <see cref="OnDrawGizmos"/> previews the result in scene view before launch.
    /// </summary>
    public abstract class SectorSpawner : MonoBehaviour
    {
        /// <summary>Ships produced during the last <see cref="Build"/> call (for injection / wiring).</summary>
