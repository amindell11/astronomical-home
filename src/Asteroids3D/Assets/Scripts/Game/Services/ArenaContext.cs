...
{
    /// <summary>
    /// Per-arena handle bundling the world providers an AI ship reads, the arena's in-plane offset,
    /// and the arena root all session content parents under. Slots are dereferenced each frame, so a
    /// field registered after ships are wired (during sector setup) is picked up live.
    /// </summary>
    public class ArenaContext
    {
        ...
        public ArenaContext(Vector2 offset, IShipRegistry registry, NavFieldService navField, Transform root)
        {
            Offset = offset;
