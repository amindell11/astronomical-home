...
{
    /// <summary>The K1-2 velrebase measurement composition: the measured ship is the archetype-under-test on the baseline slot (installed per block through the roster primitive, legacy or anchored drive, wVelTrack clone included), the enemy on the agent slot is the non-reactive <see cref="FixedCircuitBrain"/>. No ShipAgent exists — both ships are scripted, so the driver paces episodes with a null agent and every episode ends by rule, not reward.</summary>
    internal sealed class OpenLoopArchetypeComposition : ISessionComposition
    {
        ...
        {
            var moverCenter = arena.Offset;
            Pair = EpisodePair.Spawn(units, arena, projectiles, in spec, (commander, baselineShip) =>
            {
                var mover = commander.InstallBrain<FixedCircuitBrain>();
                mover.Configure(moverCenter);
                return mover;
            }, assets);
