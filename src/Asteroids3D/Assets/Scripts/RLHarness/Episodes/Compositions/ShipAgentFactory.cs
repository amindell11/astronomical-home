...
namespace Game.RLHarness
{
    ...
    public static class ShipAgentFactory
    {
        ...
        public static ShipAgent ComposeForTraining(EpisodePair pair, PolicyBrain brain,
            in RewardSpec spec, Vector2 arenaCenter, Transform parent = null) =>
            Compose(pair.Agent, pair.Baseline, brain, in spec, arenaCenter,
                BehaviorType.Default, null, teamId: 0, parent);

        public static ShipAgent ComposeHeuristicOnly(EpisodePair pair, PolicyBrain brain,
            in RewardSpec spec, Vector2 arenaCenter, Transform parent = null) =>
            Compose(pair.Agent, pair.Baseline, brain, in spec, arenaCenter,
        ...
        public static ShipAgent ComposeInferenceOnly(EpisodePair pair, PolicyBrain brain,
            in RewardSpec spec, Vector2 arenaCenter, ModelAsset model, Transform parent = null)
        {
            ...
            return Compose(pair.Agent, pair.Baseline, brain, in spec, arenaCenter,
        ...
        public static (ShipAgent agentA, ShipAgent agentB) ComposeSelfPlayPair(EpisodePair pair,
            PolicyBrain brainA, PolicyBrain brainB, in RewardSpec spec, Vector2 arenaCenter,
            ...
            ModelAsset modelB = null)
        {
            ...
            if (modelA)
                Academy.Instance.InferenceSeed = EvalProtocol.InferenceSeed;
            var agentA = Compose(pair.Agent, pair.Baseline, brainA, in spec, arenaCenter,
                behaviorType, modelA, teamId: 0, parent);
            var agentB = Compose(pair.Baseline, pair.Agent, brainB, in spec, arenaCenter,
                behaviorType, modelB, teamId: 1, parent);
            return (agentA, agentB);
        }

        private static ShipAgent Compose(Ship self, Ship opponent, PolicyBrain brain, in RewardSpec spec,
