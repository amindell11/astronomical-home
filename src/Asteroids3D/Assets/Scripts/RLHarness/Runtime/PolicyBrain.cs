...
{
    /// <summary>The policy end of the decision seam: holds the decision-boundary action (enemy-anchored facing offset + polar velocity + trigger + one-shot boost) and rebuilds the decision every Decide, naming the INJECTED opponent as its anchor (Ranger precedent — Scout's 30 m radius is blind past the 25–60 m spawn band). The policy owns aim and the engage gate: it engages the primary per its fire action and keeps the secondary disengaged, while the Gunner times the trigger; the MPC re-resolves both anchored channels per rollout step. Boost emits on exactly one tick and only if it was available as observed at the boundary (spend-now-if-ready — a cooldown expiring mid-interval must not fire a boost the policy saw as unavailable).</summary>
    public class PolicyBrain : Brain, IPolicyReadout
    {
        ...
        public override BrainDecision? Decide(AIContext ctx)
        {
            ...
            return new BrainDecision(nav, engagePrimary: action.fire, engageSecondary: false, boost: boost);
        }
    }
}
